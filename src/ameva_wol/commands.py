...
class CommandDispatcher:
    ...
    async def handle_how(self, update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
        ...
        manual = (
            ...
            "5. Smart Plug Power Control (/power)\n"
            "• `/power add <alias> <email> <password> <ip> <mac>`: Register Tapo Smart Plug + PC MAC simultaneously.\n"
            "• `/power on [alias]`: Turns on the smart plug and automatically sends a Wake-on-LAN packet.\n"
            "• `/power off [alias]`: Turns off the smart plug.\n"
            "• `/power re [alias]`: Reboots (turns off, waits, turns on, then WoL).\n"
            "• `/power status [alias]`: Shows current physical power state of the plug.\n\n"
            "6. Always-On Mode\n"
            ...
            "7. Important Hardware & Network Notes\n"
